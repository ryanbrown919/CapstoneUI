...
class ChessBoard(Widget):
    def __init__(self, **kwargs):
        ...
        self.move_log = []  # Store moves

        self.white = True

        self.build_board()
        self.add_labels(white=self.white)
        self.add_pieces(white=self.white)
        #self.add_textbox()
    ...
    def add_labels(self, white=True):
        """Add traditional chess labels around the board."""
        for i in range(8):
            # Column labels (a-h)
            column_label = Label(
                text=chr(97+i) if white else chr(97-i),
                #text=chr(97 + i),
                font_size=16,
                size_hint=(None, None),
                size=(self.square_size, 20),
                pos=(self.center_x + i * self.square_size, self.center_y - 20)
            )
            self.add_widget(column_label)

            # Row labels (1-8)
            row_label = Label(
                text=str(8-i) if white else str(i+1),  # Reverse labels for white on bottom
                font_size=16,
                size_hint=(None, None),
                size=(20, self.square_size),
                pos=(self.center_x - 20, self.center_y + (7 - i) * self.square_size)
            )
            self.add_widget(row_label)

    def add_pieces(self, white=True):
        ...
        if white:
            ...
            piece_positions = {
                ...
                'white_queen': [(0, 4)],
                'white_king': [(0, 3)],
                'white_pawn': [(1, col) for col in range(8)],
                ...
                'black_queen': [(7, 4)],
                'black_king': [(7, 3)],
                'black_pawn': [(6, col) for col in range(8)],
            }
        ...
        for piece, positions in piece_positions.items():
            for row, col in positions:
                piece_image = ChessPiece(
                    ...
                    size=(self.square_size, self.square_size),
                    pos=(self.center_x + col * self.square_size,
                         self.center_y + (7 - row) * self.square_size)  # Adjust row for white at bottom
                )
                piece_image.bind(on_press=self.select_piece)
                self.add_widget(piece_image)

    def add_textbox(self):
        """Add a text box to display move log."""
    ...
    def on_touch_down(self, touch):
        ...
        if self.selected_piece and self.collide_point(*touch.pos):
            ...
            self.selected_piece.pos = (
                ...
                self.center_y + (7 - row) * self.square_size  # Adjust for white at bottom
            )

            # Log the move in chess notation
            old_square = self.get_square_name(self.selected_piece.pos)
            new_square = self.get_square_name((col, row))
            self.move_log.append(f"{old_square}-{new_square}")
            self.text_input.text += f"{old_square}-{new_square}\n"

            # Deselect the piece after the move
            self.selected_piece.deselect()
            self.selected_piece = None

    def get_square_name(self, pos):
        """Convert position to chess notation."""
        col = chr(97 + int((pos[0] - self.center_x) / self.square_size))
        row = str(8 - int((pos[1] - self.center_y) / self.square_size))
        return f"{col}{row}"
